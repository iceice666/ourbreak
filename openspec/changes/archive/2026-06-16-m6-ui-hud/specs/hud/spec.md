...
The HUD value formatting (round counter text, countdown text, building-count text) SHALL be computed by pure functions independent of the renderer so the formatting is unit-testable.

#### Scenario: Deterministic formatting
- **WHEN** the formatting helper is given the same round, remaining-seconds, and block-count inputs
